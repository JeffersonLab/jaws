...
| active-alarms | Set of alarms currently active (alarming). | String: alarm name | AVRO: [active-alarms-value](https://github.com/JeffersonLab/jaws/blob/master/config/subject-schemas/active-alarms-value.avsc) | set-active.py, list-active.py |
...
| class-registration | Contains class-wide registration information to avoid redundant specification | AVRO: [class-registration-key](https://github.com/JeffersonLab/jaws/blob/master/config/subject-schemas/class-registration-key.avsc) | AVRO: [class-registration-value](https://github.com/JeffersonLab/jaws/blob/master/config/subject-schemas/class-registration-value.avsc) | Optionally some registration fields can be left empty and instead inherit values from a group specification - for example the corrective action for a certain class of machine Magnets (of which there are hundreds of instances) is typically the same. |
